-- PostgreSQL schema for ARS JUNCTION Food Ordering Platform
...
CREATE TABLE users (
    ...
    social_type VARCHAR(10) DEFAULT 'normal',
    is_admin BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Add a check constraint for social_type
ALTER TABLE users ADD CONSTRAINT check_social_type 
CHECK (social_type IN ('facebook', 'google', 'normal'));
...
CHECK (order_status IN ('pending', 'confirmed', 'preparing', 'on the way', 'delivered', 'cancelled'));
