...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['request_otp'])) {
        try {
            ...
            $rateLimit = RateLimiter::checkAuth($email, 'restaurant_forgot_password');
            if (!$rateLimit['allowed']) {
                ...
                if ($user) {
                    ...
                    if (send_otp_email($email, $user['name'], $otp)) {
                        ...
                        $error_msg = 'Failed to send verification email. Please verify SMTP server settings.';
                    }
                } else {
                    RateLimiter::recordAuthFailure($email, 'restaurant_forgot_password');
