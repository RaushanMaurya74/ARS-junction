...
function admin_get_all_users($limit = 100, $offset = 0) {
    global $conn;
    $sql = "SELECT * FROM users WHERE is_admin = 0 ORDER BY created_at DESC LIMIT :limit OFFSET :offset";
...
function admin_get_delivery_boys() {
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM users WHERE is_delivery_boy = 1 ORDER BY name");
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
